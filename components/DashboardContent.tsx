...
export function DashboardContent() {
  ...
  return <>
    ...
    <div className="dashboard-grid"><TokenTable data={trending} title="Live signal shortlist" kicker="DEX SCREENER MARKET DATA" compact mode="trending" /><section className="panel signal-feed"><div className="panel-head"><div><span className="panel-kicker">NEWLY OBSERVED</span><h2>Fresh candidates</h2></div><Link href="/new-tokens" className="text-button">All tokens →</Link></div><div className="signal-list">{recent.map((token) => <div key={token.id}><span className="signal-dot" /><div><strong>{token.symbol}</strong><p>{formatAge(token.ageMinutes)} old · {token.buyPressure}% buy pressure</p></div><time>{token.score} score</time></div>)}</div></section></div>
    <div className="insight-grid"><section className="panel radar-card"><div><span className="panel-kicker">CURRENT COVERAGE</span><h2>Market signals are live. On-chain checks come next.</h2><p>Price, volume, liquidity, pair age, and buy/sell activity come directly from DEX Screener in the browser. Mint authority and holder concentration remain marked as pending until Helius is connected.</p>{leader && <Link className="secondary-button" href={`/token/${leader.id}`}>Inspect a live score →</Link>}</div><div className="radar-visual"><i /><i /><i /><span>{leader?.score ?? '—'}<small>TOP SCORE</small></span></div></section>{leader && <section className="panel leader-card"><div className="panel-head"><div><span className="panel-kicker">TOP LIVE SIGNAL</span><h2>Highest score in this set</h2></div></div><Link href={`/token/${leader.id}`} className="leader-token"><TokenLogo symbol={leader.symbol} color={leader.color} imageUrl={leader.imageUrl} large /><div><strong>{leader.symbol}</strong><small>{leader.name} · {formatAge(leader.ageMinutes)} old</small></div><ScoreBadge score={leader.score} large /></Link><p><span>Why it stands out</span> This comparative score uses current activity and liquidity. It is not a recommendation or a return forecast.</p></section>}</div>
