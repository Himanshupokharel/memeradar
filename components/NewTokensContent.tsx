...
export function NewTokensContent({ query }: { query: string }) {
  ...
  return <>
    ...
    <div className="context-strip"><span>{status === 'live' ? 'LIVE · 3S MARKET ROTATION' : status === 'connecting' ? 'CONNECTING' : 'FALLBACK · RETRYING'}</span><p>Discovery refreshes five official DEX Screener channels every 15 seconds. Market metrics rotate through the complete candidate pool every {discovery?.coverageSeconds || 3} seconds.</p></div>
